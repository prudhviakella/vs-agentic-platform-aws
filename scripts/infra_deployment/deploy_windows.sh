...
#   - Run this script inside Git Bash (not CMD / PowerShell)
#   - Save this file with LF line endings (not CRLF) — use: git config core.autocrlf false
#   - Requires: aws-cli v2, docker (Docker Desktop), terraform, python3 (or python)
#   - MSYS_NO_PATHCONV=1 is set globally to prevent Git Bash from mangling
#     Unix-style paths passed as AWS CLI arguments (ARNs, file:// URIs, etc.)

set -euo pipefail

# ── Windows Git Bash: disable path conversion ─────────────────────────────
# Without these, Git Bash rewrites paths like /vs-agentcore/prod → C:/vs-agentcore/prod
# inside AWS CLI arguments, breaking SSM prefixes, ARNs, and file:// URIs.
export MSYS_NO_PATHCONV=1
export MSYS2_ARG_CONV_EXCL="*"

# ── Cross-platform temp directory ─────────────────────────────────────────
# mktemp -d works on Git Bash; fall back to /tmp if unavailable.
TMPDIR="$(mktemp -d 2>/dev/null || echo "/tmp")"

# ── Python binary (python3 on Linux/Mac, often python on Windows) ─────────
if command -v python3 &>/dev/null; then
  PY="python3"
elif command -v python &>/dev/null; then
  PY="python"
else
  echo "❌ Python not found. Install Python 3 and ensure it is on your PATH."
  exit 1
fi
...
check_prereqs() {
  ...
  # Python already validated above; just check boto3
  $PY -c "import boto3" &>/dev/null || missing+=("boto3 — run: pip install boto3")
...
# ── Step 2: IAM Roles ──────────────────────────────────────────────────────
# FIX (Windows Git Bash): IAM policy JSON is written to ${TMPDIR} (not hardcoded /tmp).
# All `aws iam` calls are prefixed with MSYS_NO_PATHCONV=1 to prevent Git Bash
# from converting the file:// URI (e.g. file:///C:/Users/... mangling) and to
# stop ARN path segments being treated as Unix paths.
...
step_iam() {
  ...
  cat > "${TMPDIR}/lambda-trust.json" << 'JSON'
{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"lambda.amazonaws.com"},"Action":"sts:AssumeRole"}]}
JSON
  MSYS_NO_PATHCONV=1 aws iam create-role --role-name "${PREFIX}-lambda-mcp" \
    --assume-role-policy-document "file://${TMPDIR}/lambda-trust.json" 2>/dev/null || true
  MSYS_NO_PATHCONV=1 aws iam attach-role-policy --role-name "${PREFIX}-lambda-mcp" \
    --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole 2>/dev/null || true

  cat > "${TMPDIR}/lambda-secrets.json" << POLICY
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
POLICY
  MSYS_NO_PATHCONV=1 aws iam put-role-policy --role-name "${PREFIX}-lambda-mcp" \
    --policy-name SecretsAccess \
    --policy-document "file://${TMPDIR}/lambda-secrets.json"
  echo "  ✅ ${PREFIX}-lambda-mcp"

  # ── MCP Gateway role ──────────────────────────────────────────────────────
  cat > "${TMPDIR}/gateway-trust.json" << 'JSON'
...
JSON
  MSYS_NO_PATHCONV=1 aws iam create-role --role-name "${PREFIX}-gateway-role" \
    --assume-role-policy-document "file://${TMPDIR}/gateway-trust.json" 2>/dev/null || true

  cat > "${TMPDIR}/gateway-policy.json" << POLICY
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
POLICY
  MSYS_NO_PATHCONV=1 aws iam put-role-policy --role-name "${PREFIX}-gateway-role" \
    --policy-name GatewayPolicy \
    --policy-document "file://${TMPDIR}/gateway-policy.json"
  echo "  ✅ ${PREFIX}-gateway-role"

  # ── AgentCore Runtime role ────────────────────────────────────────────────
  cat > "${TMPDIR}/agentcore-trust.json" << 'JSON'
{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"bedrock-agentcore.amazonaws.com"},"Action":"sts:AssumeRole"}]}
JSON
  MSYS_NO_PATHCONV=1 aws iam create-role --role-name "${PREFIX}-agent-role" \
    --assume-role-policy-document "file://${TMPDIR}/agentcore-trust.json" 2>/dev/null || true

  cat > "${TMPDIR}/agentcore-policy.json" << POLICY
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
POLICY
  MSYS_NO_PATHCONV=1 aws iam put-role-policy --role-name "${PREFIX}-agent-role" \
    --policy-name AgentCorePolicy \
    --policy-document "file://${TMPDIR}/agentcore-policy.json"
...
step_lambdas() {
  ...
  # FIX (Windows Git Bash): --image-config value pre-assigned to avoid
  # single-quote / escape issues inside the aws lambda create-function call.
  LAMBDA_IMAGE_CONFIG='{"Command":["handler.handler"]}'
  ...
  for tool in search graph hitl summariser; do
    ...
    if aws lambda get-function --function-name "${FUNC}" --region "${REGION}" &>/dev/null; then
      ...
      aws lambda create-function \
        ...
        --timeout 30 \
        --memory-size 512 \
        --image-config "${LAMBDA_IMAGE_CONFIG}" \
    ...
    RESULT=$(aws lambda invoke --function-name "${FUNC}" \
      ...
      --region "${REGION}" "${TMPDIR}/lambda_out.json" 2>/dev/null && cat "${TMPDIR}/lambda_out.json")
...
step_gateway() {
  ...
  if [ -n "${EXISTING_GW}" ] && [ "${EXISTING_GW}" != "None" ]; then
    ...
    GATEWAY_ID=$(echo "${RESPONSE}" | $PY -c "import sys,json; print(json.load(sys.stdin)['gatewayId'])")
    GATEWAY_URL=$(echo "${RESPONSE}" | $PY -c "import sys,json; print(json.load(sys.stdin)['gatewayUrl'])")
...
step_agent() {
  ...
  if [ -n "${EXISTING_ARN}" ] && [ "${EXISTING_ARN}" != "None" ]; then
    ...
    # FIX (Windows Git Bash): use | sed 's|.*runtime/||' instead of s/.*runtime\///
    # The double-slash // in the original sed pattern can confuse Git Bash's path
    # expansion; using an alternate delimiter | avoids this entirely.
    ...
    RUNTIME_ARN=$(echo "${RUNTIME_RESPONSE}" | $PY -c \
    ...
    # FIX (Windows Git Bash): alternate sed delimiter — see note above
